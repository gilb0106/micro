...
public class UserDao implements UserRepo {
    ...
    @Override
    public Optional<User> findByEmail(String email) { // new method to help with associating github login to user in ums
        String sql = "SELECT u.id, u.name, u.email, u.password, u.created, " +
                "GROUP_CONCAT(DISTINCT r.id ORDER BY r.name SEPARATOR ', ') AS roleids, " +
                "GROUP_CONCAT(DISTINCT r.name ORDER BY r.name SEPARATOR ', ') AS rolenames, " +
                "GROUP_CONCAT(DISTINCT r.description ORDER BY r.name SEPARATOR ', ') AS roledescriptions, " +
                "lv.in AS last_session " +
                ...
                "LEFT JOIN last_visit lv ON u.last_visit_id = lv.id " +
                "WHERE u.email LIKE ? " +
                "GROUP BY u.id, u.name, u.email, u.password, u.created, lv.in";

        // Use query to get a list of results, even though we expect only one result
        List<User> users = jdbcTemplate.query(sql, new Object[]{email + "%"}, (rs, rowNum) -> {
            User user = new User();
            user.setId(BinaryConverter.convertBytesToUUID(rs.getBytes("id"))); // Assuming id is stored as binary
            user.setName(rs.getString("name"));
            user.setEmail(rs.getString("email"));
            user.setPassword(rs.getString("password"));
            long createdTimestamp = rs.getLong("created");
            user.setCreated(new Timestamp(createdTimestamp)); // Convert to LocalDateTime
